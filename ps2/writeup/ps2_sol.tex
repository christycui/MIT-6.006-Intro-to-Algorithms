...
We know that at level i for 0 <= i < n, there are 3 * 4^i nodes. Each node render a triangle of size ((1/3)^(i + 1))^2. This is because there is no triangle rendering at the last level, the smallest triangles are rendered at n-1 level. Therefore, the rendering time at level i is 3 * 4^i * (1/9)^(i+1), which can be simplified into \Theta((4/9)^i). The sum of rendering at all levels is therefore bounded by 1.
